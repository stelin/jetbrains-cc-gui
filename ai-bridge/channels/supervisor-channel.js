...
function buildSystemPrompt({ name, description, planContent, specContent }) {
    ...
    sections.push(
        ...
        '',
        '# 调度状态机不变量（所有监督者强制）',
        '`approve_and_continue(mark_step_complete=N)` 只表示 Step N 已通过并可关闭；它**不会**自动派发 Step N+1，也**不会**创建新的 MAIN_AI contract。',
        '因此每轮 `emit_action` 前必须按顺序检查：',
        '1) 所有 step 都处于 done/skipped/blocked → 必须 `emit_action(action="complete_plan", summary="...")`，禁止 wait。',
        '2) 存在已派发但未收到 turn_report / turn_end 的 step → 必须 `emit_action(action="wait_for_contract", contractId="...")`；若没有真实 OPEN contract，按真实状态重派当前步或升级，禁止假装等待。',
        '3) 存在未完成且未派发的 next step，且当前没有 OPEN MAIN_AI contract → 必须 `emit_action(action="inject_prompt", ...)` 派发 next step。',
        '4) 只有没有可完成、可等待、可派发的动作时，才允许 `emit_action(action="wait")`。',
        '在“plan 未完成 + 无 OPEN MAIN_AI contract + 有未派发 next step”状态下，禁止只回答自然语言、禁止 wait、禁止等待 DISPATCH_NEXT_STEP、禁止说“还没完成/继续中”但不派单。',
        '',
        '# 用户状态问题处理（也必须推进状态机）',
        '当用户问“完成了吗 / 任务完成了吗 / 进度怎么样 / 到哪了 / 继续 / 卡住了吗 / 下一步是什么”时，不得只做自然语言回答。',
        '必须先按上面的调度状态机选择 action，并让自然语言与 action 一致：',
        '- 已全部完成 → `complete_plan`，然后说明已完成；',
        '- 有 OPEN MAIN_AI contract → `emit_action(action="wait_for_contract", contractId="...")`，然后说明正在等待主 AI；',
        '- 还有未派发 next step 且无 OPEN MAIN_AI contract → `inject_prompt`，然后说明还未完成，已继续派发 Step N。',
        '自然语言说“已派发/继续派发/下发”时，本轮 action 必须是 `inject_prompt`；说“收尾/已完成”时必须是 `complete_plan`；说“等待主 AI”时必须是 `wait_for_contract`。',
        '',
        '# 你可用的只读工具（必读）',
        ...
        '',
        '若不确定下一步，先按「调度状态机不变量」检查：可派发就 `inject_prompt`，可收尾就 `complete_plan`，有真实 OPEN MAIN_AI contract 才 `wait_for_contract`；只有确实无可派发、无可收尾、无可等待时才 `wait`。**绝不可以只输出文字不调用工具**。',
        '',
        '# 行为约束',
