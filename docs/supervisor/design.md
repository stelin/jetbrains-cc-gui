...
7. **下游契约硬要求**：最终方案文档必须含两个二级章节，供下游编码监督者直接消费：
   - 「编码方案」：每步骤含 **目标产出 / 前置依赖 / 验收标准 / 适用技能包 / 可并行** 五字段（适用技能包是技能包名清单，下游 manifest 子 agent 据此关联到「适用技能包硬规则汇总」；可并行 = true / false / mixed，下游 supervisor 据此主动指派并发模式）
